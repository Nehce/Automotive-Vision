...
% Performs the resampling step of the particle set.

particles_resampled = zeros(size(particles));

num_particles = size(particles, 1);
normalized_weights = particles(:,1) ./ sum(particles(:,1), 1); % sum(A,2) 是包含每一列总和的行向量。
...
for i = 1:num_particles
    amount = indices(i);  % number of repetitions
    if (amount > 0)
