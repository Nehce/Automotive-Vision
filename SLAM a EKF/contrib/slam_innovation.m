...
for i=1:length(observations) % length(observations) 观测到了几个Landmarks?
    id = observations(i).id;
    if (id > num_landmarks)        
        %%% Implement your code here (1. Dynamic extension of state, covariance matrix, oberservation matrix H)
        no_ext_state = (id-num_landmarks)*2; % number of extend state
        state = [state; zeros(no_ext_state,1)];                               
        cov = [cov, zeros(3+2*num_landmarks, no_ext_state); ...
               zeros(no_ext_state,3+2*num_landmarks), 1e10*eye(no_ext_state)];                        
    ...
    d = state(2+2*id:3+2*id)-state(1:2); % (𝑙𝑥−𝑥) (𝑙𝑦−𝑦)
