...
Gdir = atan2(Gv, Gu)*180/pi+180; % the gradient direction in degree between -180 and 180 for each pixel
Gbin = mod(floor(Gdir/45), 8)+1; 
%{ 
the direction bin to which each pixel belongs 
(mod is necessary to overcomenerrors for angles of +180.00 
%}
for v = 1:size(Gu, 1)
    for u = 1:size(Gu,2)
