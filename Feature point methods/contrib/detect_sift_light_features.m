...
for i = 1:num_scales
    ...
    % -----(task 3.1a) ------
    hsize = 2*round(scale*[5 5])+1;
    LoG = fspecial('log', hsize, scale);
...
% return of local_maxima is a n-by-4 matrix that contains the coordinates 
% of one feature per row and the L-value
features_all = local_maxima (L);
features_all (:,5) = scales (features_all(:,3))'; % 增加第五列，为scale(σ)的值
...
features_clean = features(idx_select==6,:); % 用一个bool vector确定是否要将feature的行加入feature_clean
                                            % Python如何实现？
end
...
features_clean = features(idx_select==1,:);
...
for i=1:n
    ...
    u = features(i, 2);
    s = features(i, 3);
    Dxx = L(v+1, u, s) -2*L(v,u,s) + L(v-1,u,s);
    Dyy = (L(v+1,u,s) - 2*L(v,u,s) + L(v-1,u,s));
    Dxy = (1/4)*(L(v+1,u+1,s) + L(v-1, u-1, s)-L(v+1,u-1,s)-L(v-1,u+1,s));
    crit(i) = (Dxx+Dyy)^2/(Dxx*Dyy-Dxy^2);
end
